...
def update_b_cost(span, bridge_type):
    ...
    span_independent_costs = (
        cost[bridge_type]["a_concrete_works"]
        + cost[bridge_type]["b_steel_reinforcement"]
        + cost[bridge_type]["c_tower_system"]
        + cost[bridge_type]["f_ramp_post_system"]
        + cost[bridge_type]["i_temporary_works"]
        + cost[bridge_type]["j_misc_building_materials"]
        + cost[bridge_type]["k_sign_for_bridge"]
        + cost[bridge_type]["l_certificates_manuals_printing"]
        + cost[bridge_type]["m_safety_equipment"]
        + cost[bridge_type]["n_scaffolding_rental"]
        + cost[bridge_type]["o_tools"]
        + cost[bridge_type]["p_transportation_site_setup"]
        + cost[bridge_type]["q_survey_warehouse_misc"]
        + cost[bridge_type]["r_laborers_operatives"]
        + cost[bridge_type]["s_site_supervision_foreman_eng"]
        + cost[bridge_type]["t_expenses"]
    )
    span_dependent_costs = (
        cost[bridge_type]["d_steel_decking_crossbeams"]
        + cost[bridge_type]["e_fencing_system"]
...
def create_chart(exchange, span_units):
    ...
    fig.update_layout(  # Hide the mode bar
        ...
        font=dict(
            family="Source Sans Pro", size=14, color="black"
...
def data_category(bridge_type):
    categories = {
        "materials": cost[bridge_type]["a_concrete_works"]
        + cost[bridge_type]["b_steel_reinforcement"]
        + cost[bridge_type]["c_tower_system"]
        + (cost[bridge_type]["d_steel_decking_crossbeams"] * span)
        + (cost[bridge_type]["e_fencing_system"] * span)
        + cost[bridge_type]["f_ramp_post_system"]
        + (cost[bridge_type]["g_restraint_handrail_wires"] * span)
        + cost[bridge_type]["j_misc_building_materials"]
        + cost[bridge_type]["k_sign_for_bridge"]
        + cost[bridge_type]["l_certificates_manuals_printing"],
        "in_kind": (cost[bridge_type]["h_cables_clips"] * span),
        "tools_and_equipment": cost[bridge_type]["i_temporary_works"]
        + cost[bridge_type]["m_safety_equipment"]
        + cost[bridge_type]["n_scaffolding_rental"]
        + cost[bridge_type]["o_tools"]
        + cost[bridge_type]["p_transportation_site_setup"],
        "labour": cost[bridge_type]["r_laborers_operatives"],
        "supervision": cost[bridge_type]["s_site_supervision_foreman_eng"]
        + cost[bridge_type]["t_expenses"],
        "overheads": cost[bridge_type]["q_survey_warehouse_misc"]
        + cost[bridge_type]["v_b2p_staff_inc_engineering"]
        + cost[bridge_type]["w_indirect_company_costs"],
...
with header_col1:
    ...
    st.markdown(
        "This tool estimates the cost of building a bridge based on the span and the type of bridge. Note, this was produced using data from 2021 - April 2023 and all numbers are a general estimate. Please [contact us](https://fika.org/contact/) with questions or to learn more about our methodology"
    )
with header_col2:
...
with tab1:
    ...
    with col1:
        ...
        span = st.number_input(
            "Estimated bridge span (m)", 40, 120, 90, 1, key="span_input_suspension"
        )
        exchange_rate = st.number_input(
            "RWF to USD exchange rate",
            0,
            1000000,
            1459,
            1,
            key="exchange_rate_both_suspension",
        )
        ...
        data = [
            {
                "Category": "Materials",
                "Cost RWF": suspension_data["materials"] - 8200000,
                "Cost USD": (suspension_data["materials"] - 8200000) / exchange_rate,
                "Percentage": str( round((suspension_data["materials"] / bridge_cost) * 100) ) + "%",
            },
            {
                "Category": "In-Kind",
                "Cost RWF": suspension_data["in_kind"] + 8200000,
                "Cost USD": (suspension_data["in_kind"] + 8200000) / exchange_rate,
                "Percentage": str( round((suspension_data["in_kind"] / bridge_cost) * 100) ) + "%",
            },
            {
                "Category": "Tools & Equipment",
                "Cost RWF": suspension_data["tools_and_equipment"],
                "Cost USD": suspension_data["tools_and_equipment"] / exchange_rate,
                "Percentage": str( round((suspension_data["tools_and_equipment"] / bridge_cost) * 100) ) + "%",
            },
            {
                "Category": "Labour",
                "Cost RWF": suspension_data["labour"],
                "Cost USD": suspension_data["labour"] / exchange_rate,
                "Percentage": str( round((suspension_data["labour"] / bridge_cost) * 100) ) + "%",
            },
            {
                "Category": "Supervision",
                "Cost RWF": suspension_data["supervision"],
                "Cost USD": suspension_data["supervision"] / exchange_rate,
                "Percentage": str( round((suspension_data["supervision"] / bridge_cost) * 100) ) + "%",
            },
            {
                "Category": "Overheads",
                "Cost RWF": suspension_data["overheads"],
                "Cost USD": suspension_data["overheads"] / exchange_rate,
                "Percentage": str( round((suspension_data["overheads"] / bridge_cost) * 100) ) + "%",
            },
            {
                "Category": "Total",
                "Cost RWF": bridge_cost,
                "Cost USD": round(bridge_cost / exchange_rate),
                "Percentage": "",
            },
        ]
    ...
    with col2:
        ...
        with cola:
            span_units = st.radio(
                "Cost per meter or total cost?",
                ("Total cost", "Cost per meter"),
                key="span_units_suspension",
            )
        with colb:
...
with tab2:
    ...
    with col3:
        ...
        span = st.number_input(
            "Estimated bridge span (m)", 40, 120, 90, 1, key="span_input_suspended"
        )
        exchange_rate = st.number_input(
            "RWF to USD exchange rate",
            0,
            ...
            1459,
            1,
            key="exchange_rate_both_suspended",
        )
        ...
        data = [
            {
                "Category": "Materials",
                "Cost RWF": suspended_data["materials"],
                "Cost USD": suspended_data["materials"] / exchange_rate,
                "Percentage": str(
                    round((suspended_data["materials"] / bridge_cost) * 100)
                )
                + "%",
            },
            {
                "Category": "In-Kind",
                "Cost RWF": suspended_data["in_kind"],
                "Cost USD": suspended_data["in_kind"] / exchange_rate,
                "Percentage": str(
                    round((suspended_data["in_kind"] / bridge_cost) * 100)
                )
                + "%",
            },
            {
                "Category": "Tools & Equipment",
                "Cost RWF": suspended_data["tools_and_equipment"],
                "Cost USD": suspended_data["tools_and_equipment"] / exchange_rate,
                "Percentage": str(
                    round((suspended_data["tools_and_equipment"] / bridge_cost) * 100)
                )
                + "%",
            },
            {
                "Category": "Labour",
                "Cost RWF": suspended_data["labour"],
                "Cost USD": suspended_data["labour"] / exchange_rate,
                "Percentage": str(round((suspended_data["labour"] / bridge_cost) * 100))
                + "%",
            },
            {
                "Category": "Supervision",
                "Cost RWF": suspended_data["supervision"],
                "Cost USD": suspended_data["supervision"] / exchange_rate,
                "Percentage": str(
                    round((suspended_data["supervision"] / bridge_cost) * 100)
                )
                + "%",
            },
            {
                "Category": "Overheads",
                "Cost RWF": suspended_data["overheads"],
                "Cost USD": suspended_data["overheads"] / exchange_rate,
                "Percentage": str(
                    round((suspended_data["overheads"] / bridge_cost) * 100)
                )
                + "%",
            },
            {
